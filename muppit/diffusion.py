  ...
  def _cbg_denoise(
    ...
    if cache is not None:
      ...
      if use_approx:
        ...
        # 
        # (bsz, binder_length), (bsz, target_length)
        ...
        # neg_classifier_log_prob = torch.log(bindevaluator_probs) * (1-mask_vec)
    

        # pdb.set_trace()
        classifier_log_prob = classifier_log_prob.sum(dim=-1) / mask_vec.sum()
        # neg_classifier_log_prob = neg_classifier_log_prob.sum(dim=-1) / (1-mask_vec).sum()
        # weighting = torch.clamp(classifier_log_prob / neg_classifier_log_prob, max=1)
        
        classifier_log_prob = classifier_log_prob.reshape(bsz, seq_len, self.vocab_size)

        # classifier_log_prob = log((cosine_similarity(ESM2(xt_jumps), embedding) + 1) / 2)

        # cosine similarity -> [-1, 1] -> [0, 1] -> log()
        # ESM similarity guidance
        # original sequence

        # BindEvaluator: binder, target -> binding sites
