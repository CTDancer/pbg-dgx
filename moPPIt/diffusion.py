...
class Diffusion(L.LightningModule):
  ...
  def sample(
    ...
    if self.parameterization == 'ar':
      ...
      # self.pepmlm = AutoModelForMaskedLM.from_pretrained("ChatterjeeLab/PepMLM-650M").to(self.device)
      # self.pepmlm_tokenizer = AutoTokenizer.from_pretrained("ChatterjeeLab/PepMLM-650M")
  ...
  def _cbg_denoise(
    ...
    if cache is not None:
      ...
      if use_approx:
        ...
        # Compute PPL
        # ppl = compute_pseudo_perplexity_batch(self.pepmlm, self.pepmlm_tokenizer, target_sequence.repeat(xt_jumps.shape[0], 1), xt_jumps)
        # ppl = ppl.reshape(bsz, seq_len, self.vocab_size)

        # classifier_log_prob = scaling(classifier_log_prob, ppl)
...
def compute_pseudo_perplexity_batch(model, tokenizer, protein_seq, binder_seq):
    """
    Compute pseudo perplexity for a batch of protein and binder sequences.
    
    Args:
        model: Pretrained model.
        tokenizer: Tokenizer used for encoding sequences.
        protein_seq (torch.Tensor): Tensor of protein sequences (B, L2).
        binder_seq (torch.Tensor): Tensor of binder sequences (B, L1).
    
    Returns:
        pseudo_perplexities (list): List of pseudo perplexities for each sequence in the batch.
    """
    batch_size, binder_len = binder_seq.shape
    pseudo_perplexities = []

    # Concatenate protein and binder sequences for each batch
    concatenated_seq = torch.cat([protein_seq, binder_seq], dim=1)  # Shape: (B, L2 + L1)
    device = model.device

    # Iterate over each sequence in the batch
    for batch_idx in range(batch_size):
        sequence = concatenated_seq[batch_idx].unsqueeze(0)  # Shape: (1, L2 + L1)
        total_loss = 0

        # Loop through each token in the binder sequence
        for i in range(-(binder_len-2)-1, -1):
            ...
            # Create labels
            labels = torch.full(sequence.shape, -100).to(device)
            labels[0, i] = sequence[0, i]
    ...
    return torch.tensor(pseudo_perplexities)

def scaling(classifier_log_prob, ppl):
    """
    Scale down classifier_log_prob according to ppl.
    The bigger ppl is, the more negative the log probabilities become
    (once re-normalized), and vice versa.
    
    Args:
        classifier_log_prob: Tensor of shape (B, L, N) containing log probabilities (all negative).
        ppl: Tensor of the same shape (B, L, N) containing perplexities in [1, +∞).
    
    Returns:
        new_classifier_log_prob: Tensor of shape (B, L, N) of valid log probabilities,
                                 scaled according to ppl.
    """
    
    # "Scale" (treating original log-probs as logits)
    ppl = ppl.to(classifier_log_prob.device)
    ppl = ppl / torch.min(ppl) * 5.0

    scale = torch.where(ppl < 5, torch.ones_like(ppl), 5.0 / ppl)
    scaled_logits = classifier_log_prob / scale
    
    # Convert back to valid log probabilities via log_softmax.
    new_classifier_log_prob = F.log_softmax(scaled_logits, dim=-1)
    return new_classifier_log_prob