...
class WrapBinderESM(nn.Module):
  def __init__(self, bert_model_path):
    super(WrapBinderESM, self).__init__()
    #self.bert_model_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #self.model = AutoModelForMaskedLM.from_pretrained(bert_model_path).to(self.bert_model_device)
    self.model = AutoModelForMaskedLM.from_pretrained(bert_model_path, device_map='cpu')
    self.tokenizer = AutoTokenizer.from_pretrained(bert_model_path)
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    elif self.config.backbone == 'binder_esm_finetune':
      self.backbone = WrapBinderESM(bert_model_path=self.config.checkpointing.pretrained_esm_mdlm_automodel_path)
      self.backbone.unfreeze_all_layers()
      # self.backbone = torch.compile(self.backbone)
  ...
  def configure_optimizers(self):
    ...
    scheduler = CosineWarmup(optimizer,
                            ...
                            #  warmup_steps=self.total_steps // 10,
  ...
  #   """Computes the noisy sample xt, masking tokens only after the first occurrence of token '30' and before padding.
    
  #   Args:
  #     x: int torch.Tensor with shape (batch_size, diffusion_model_input_length), input. 
  #     move_chance: float torch.Tensor with shape (batch_size, 1).
  #   """
  #   token_30_positions = (x == 30).float() * torch.arange(x.size(1), device=x.device)
  #   first_30_indices = token_30_positions.max(dim=1, keepdim=True)[1]  
    
  #   sequence_indices = torch.arange(x.size(1), device=x.device).unsqueeze(0)
  #   mask_after_30 = sequence_indices > first_30_indices
  #   non_padding_mask = x != 1
  #   valid_mask = mask_after_30 & non_padding_mask

  #   subseq_lengths = valid_mask.sum(dim=1, keepdim=True)
  #   max_mask_length = (subseq_lengths * 1).long()

  #   move_indices = (torch.rand(*x.shape, device=x.device) < move_chance) & valid_mask
  ...
  def q_xt(self, x, move_chance):
    ...
    max_mask_length = (actual_seq_length * 0.98).long()
  ...
  def _ddpm_update(self, x, t, dt, attention_mask=None):
    sigma_t, _ = self.noise(t)
  ...
  def _sample(self, num_steps=None, eps=1e-5, x_input = None):
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ddpm':
        x = self._ddpm_update(x, t, dt, attention_mask=attention_mask)
  ...
  def _loss(self, x0, attention_mask, mask=None):
    ...
    # input_tokens=x0, output_tokens=None, attention_mask=attention_mask
